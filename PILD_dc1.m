...
dim=size(trainbpca15,2)-2;
%dim=8;
sign0=[];
k0=1;
c=size(ntr,1);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\basicdata_trainbpca15.txt','w');
...
for r=1:c-1,
    for t=r+1:c,
        ...
        d=d0;
        a0=[];
        a0(:,2:dim+1)=trainbpca15(ntr(r,1):ntr(r,2),1:dim);
        ...
        a0(p0+1:p0+q0,2:dim+1)=trainbpca15(ntr(t,1):ntr(t,2),1:dim);        
        if q0==1
...
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\w0.txt','w');
    fprintf(fid,'%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n',w2');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\theta0.txt','w');
    fprintf(fid,'%.8f\n',theta2');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\w.txt','w');
    fprintf(fid,'%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n',w4');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\theta.txt','w');
    fprintf(fid,'%.8f\n',theta4');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\w1.txt','w');
    fprintf(fid,'%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n',w6');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\theta1.txt','w');
    fprintf(fid,'%.8f\n',theta6');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\w2.txt','w');
    fprintf(fid,'%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n',w8');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\theta2.txt','w');
    fprintf(fid,'%.8f\n',theta8');
fclose(fid);
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\sign0.txt','w');
    fprintf(fid,'%g\n',sign0');
fclose(fid);
...
% For the trainbpca15 set.
load D:\highimbalance\shuttle\trainbpca15.txt;
load D:\highimbalance\shuttle\ntr.txt;
load D:\highimbalance\shuttle\inv_dc1\result\w0.txt;
load D:\highimbalance\shuttle\inv_dc1\result\w.txt;
load D:\highimbalance\shuttle\inv_dc1\result\w1.txt;
load D:\highimbalance\shuttle\inv_dc1\result\w2.txt;
load D:\highimbalance\shuttle\inv_dc1\result\theta0.txt;
load D:\highimbalance\shuttle\inv_dc1\result\theta.txt;
load D:\highimbalance\shuttle\inv_dc1\result\theta1.txt;
load D:\highimbalance\shuttle\inv_dc1\result\theta2.txt;
load D:\highimbalance\shuttle\inv_dc1\result\sign0.txt;
...
%        fprintf(fid,'(r,t)=%d\n',r,t);
        ...
        err_trainbpca15(r,t-1)=100*(1-(numtr0(1)+numtr0(2))/(p0+q0));
        err_trainbpca15(r,c+t-2)=100*((p0-numtr0(1))/p0+(q0-numtr0(2))/q0)/2;
        err_trainbpca15(r,2*c+t-3)=100*sqrt(((p0-numtr0(1))/p0)*((q0-numtr0(2))/q0));
        err_trainbpca15(r,3*c+t-4)=100*area_trainbpca15(r,t);
        err_trainbpca15(r,4*c+t-5)=(err_trainbpca15(r,t-1)+err_trainbpca15(r,c+t-2)+err_trainbpca15(r,2*c+t-3)+area_trainbpca15(r,t)*100)/4;
    end
end

err_trainbpca15
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\err_trainbpca15.txt','w');
    fprintf(fid,'%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n',err_trainbpca15');
fclose(fid);
...
%        fprintf(fid,'(r,t)=%d\n',r,t);
        ...
        err_testbpca15(r,t-1)=100*(1-(numtr0(1)+numtr0(2))/(p0+q0));
        err_testbpca15(r,c+t-2)=100*((p0-numtr0(1))/p0+(q0-numtr0(2))/q0)/2;
        err_testbpca15(r,2*c+t-3)=100*sqrt(((p0-numtr0(1))/p0)*((q0-numtr0(2))/q0));
        err_testbpca15(r,3*c+t-4)=100*area_testbpca15(r,t);
        err_testbpca15(r,4*c+t-5)=(err_testbpca15(r,t-1)+err_testbpca15(r,c+t-2)+err_testbpca15(r,2*c+t-3)+area_testbpca15(r,t)*100)/4;
    end
end

err_testbpca15
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\err_testbpca15.txt','w');
    fprintf(fid,'%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n',err_testbpca15');
fclose(fid);
...
% Majority vote for the trainbpca15ing set
...
for i=1:n0,
    re0=trainbpca15(i,1:dim)*w0'+theta0';
    re1=trainbpca15(i,1:dim)*w'+theta';
    re2=trainbpca15(i,1:dim)*w1'+theta1';
    re3=trainbpca15(i,1:dim)*w2'+theta2';
    n00=0;
    for j=1:c-1,
        for k=j+1:c,
            n00=n00+1;
...
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\Number_error_trainbpca15.txt','w');
    fprintf(fid,'\nFor the trainbpca15 set\n');
    fprintf(fid,'\n%g %g %g %g %g %g %g\n',nerr');
    fprintf(fid,'\n%g %g %g %g %g %g %g\n',nerr1');
    fprintf(fid,'\n%g\n',sum(nerr'));
...
%clear all;
%t0=clock;
% Majority vote for the testbpca15 set
...
for i=1:n0,
    re0=testbpca15(i,1:dim)*w0'+theta0';
    re1=testbpca15(i,1:dim)*w'+theta';
    re2=testbpca15(i,1:dim)*w1'+theta1';
    re3=testbpca15(i,1:dim)*w2'+theta2';
    n00=0;
    for j=1:c-1,
        for k=j+1:c,
            n00=n00+1;
...
fid=fopen('D:\highimbalance\shuttle\inv_dc1\result\Number_error_testbpca15.txt','w');
    fprintf(fid,'\nFor the testbpca15 set\n');
    fprintf(fid,'\n%g %g %g %g %g %g %g\n',nerr');
    fprintf(fid,'\n%g %g %g %g %g %g %g\n',nerr1');
    fprintf(fid,'\n%g\n',sum(nerr'));
